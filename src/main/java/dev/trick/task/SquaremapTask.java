...
public final class SquaremapTask extends BukkitRunnable {
    ...
    void updateClaims() {
        ...
        if (topLevelClaims != null) {
            topLevelClaims.stream()
                .filter(claim -> claim.getGreaterBoundaryCorner() != null)
                .filter(claim -> claim.getGreaterBoundaryCorner().getWorld() != null)
    ...
    private static String getNames(List<String> list) {
        ...
        for (String str : list) {
            try {
                ...
                names.add(offlinePlayer.getName() != null ? offlinePlayer.getName() : str);
            } catch (Exception e) {
                names.add(str);
            }
        }
    ...
    private String ownerName(Claim claim) {
        if (claim.isAdminClaim()) {
            return this.plugin.config().stringsPublic();
        }

        final String ownerName = claim.getOwnerName();
        return ownerName == null || ownerName.isBlank() ? this.plugin.config().stringsPublic() : ownerName;
    }
